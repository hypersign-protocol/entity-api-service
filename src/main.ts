...
const cluster: Cluster = c as unknown as Cluster;
import { cpus } from 'os';
async function bootstrap() {
  const numCPUs = cpus().length;

  if (cluster.isPrimary) {
    Logger.log(`Master process PID: ${process.pid}`, 'Bootstrap');
    Logger.log(`Forking ${numCPUs} workers...`, 'Bootstrap');
    for (let i = 0; i < numCPUs; i++) {
      cluster.fork(); // Forking each worker
    }
    ...
    try {
      ...
      const tenantDocuments = SwaggerModule.createDocument(
        app,
        tenantDocConfig,
        {
          include: [
